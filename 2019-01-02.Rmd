...
transcriptData%>%
mutate(
  系別=str_sub(學號,5,6))->transcriptData
```
...
```{r ans16}
transcriptData[transcriptData$`必選修類別（必∕選∕通）`=="通",]->transcriptData
transcriptData$科目名稱%>%unique-> "通識科目總表"
...
```{r ans17}
transcriptData$科目名稱 %>% as.factor %>% levels -> class

for(i in seq_along(class)){
 # 1
  group_i <- group[i] 
  # 2
  numericalIndexForSubsample<-which(transcriptData$科目名稱==group_i)
  group_i <- transcriptData[numericalIndexForSubsample,]
  # 3
  class_i <- nrow(group_i)
  # 4
  class[[i]]<-class_i
}
```
